...
//                 SP.Actual_Yd, SP.TotalLayer, SP.RejectRoll, SP.MatResidue, F.qActYard, F.qActKgs, F.qActWidth, CP.FAB_WIDTH");
    ...
    {
        $arr = array();
        $query = $this->db->query("SELECT QRNumber, qSJSupplier, qReceiveDate, qOrderNo, qStyle, qSeason, qPO, qProdCode, qProdDesc, qColorCode, qColorDesc, qFabType, qLot, fBlueBin, fLocation, qStockKgs, qStockYard, 0 as [PlanUseQtyKG],  0 as [PlanUseQtyYd]
        FROM mFabricRoll
        WHERE qProcess = 'WAREHOUSE'");
        foreach($query->result_object() as $rows )
        {
            $arr[] = $rows;
