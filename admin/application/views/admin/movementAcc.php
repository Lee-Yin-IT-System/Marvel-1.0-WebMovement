...
<style>
		 ...
		 $(document).ready(function() {
			  ...
			  var taskb = document.documentElement.clientHeight;
			   ...
			   document.getElementById("refresh").style.display = 'none';
				...
				$('#search').on('click', function(){
					...
					$('#grid').height(hgt+27)
					 srcgrid(fromdate,todate,statussage,qhour,ssource);
				...
				$('#export').on('click', function(){
					...
					{//$(location).prop('href', '<?php echo base_url(); ?>cmovement/createExcel?arr=' + arr);
					 $.redirect('<?php echo base_url(); ?>cmovementAcc/createExcel?arr=' + arr,'' , "POST", "_blank");
					}
				...
				function loadgrid(fromdate,todate,qhour)
				{
					...
					$("#grid").kendoGrid({
						dataSource: {
								...
								read: 
									{
										url: "<?php echo base_url(); ?>cmovementAcc/hdr",
			...
			editable: true, 
						...
						dataBound: function (e) {
							...
							$(".grpsage").bind("change", function (e) {
								if ($(".grpsage").is(":checked")) {
									flaggroup2 = 1;
								} else { flaggroup2 = 0;}
							});
							   ...
							   $(".chk").bind("change", function(e) {
								...
								$(".SageStatus").bind("change", function(e) {
									...
									for (var i = 0; i < ds1.length; i++) 
										...
										/*else {dataItem.SageStatus = 'checked';
											totchek++; }*/
										}
										$('#grid').data('kendoGrid').refresh();
						...
						{ title:"<input name='chkgrp' id='chkgrp' class='chkgrp' type='checkbox' data-bind='checked: chkgrp' #= chkgrp ?/>  Check",width:75,headerAttributes: {style: "font-weight: bold"} ,editable: "false",
						 ...
						 {field: "QtyKgs",title: "Stock Kgs",width: 95,editable: "false",
						  ...
						  footerTemplate: "<div style='text-align:right'> #: kendo.toString(sum, 'n2') # </div>"},
						 {field: "Qty",title: "Stock",width: 95,editable: "false",
						  headerAttributes: {style: "font-weight: bold"},
						  attributes: { style: "text-align: Right; font-size: 14px"},format:"{0:n2}",
						  footerTemplate: "<div style='text-align:right'> #: kendo.toString(sum, 'n2') # </div>"},
						 {field: "qLocImport",title: "LocImport",width: 140,hidden:false,headerAttributes: {style: "font-weight: bold"}},
						 ...
						 { title:"<input name='grpsage' id='grpsage' class='grpsage' type='checkbox' data-bind='checked: grpsage' #= grpsage ?/>Sage Status",width:105,headerAttributes: {style: "font-weight: bold"} ,editable: "false",
				...
				function srcgrid(fromdate,todate,statussage,qhour,ssource)
				{
					flag = 0;
					$("#grid").kendoGrid({
						dataSource: {
								transport: {
									read: 
										{
											url: "<?php echo base_url(); ?>cmovementAcc/srchdr",
											contentType: "application/json; charset=utf-8",
											dataType: "json",
											type: 'get',
											data:{fromdate:fromdate,todate:todate,statussage:statussage,qhour:qhour,ssource:ssource},
										},			
										},
										schema: {data: "data",
														model: {
						...
						dataBound: function(e) {
							...
							$(".chk").bind("change", function (e) {
								...
								//edit yang ini buat RN gruop auto ceklis banyak guntur
											if (checkbox.is(":checked")) 
											{
                       ...
                       /* dataBound: function() {
						...
						columns: [
						 ...
						 {field: "Qty",title: "Stock",width: 95,editable: "false",
						  ...
						  attributes: { style: "text-align: Right; font-size: 14px"},format:"{0:n2}",
						  footerTemplate: "<div style='text-align:right'> #: kendo.toString(sum, 'n2') # </div>"},
						 {field: "qLocImport",title: "Local Import",width: 140,hidden:false,headerAttributes: {style: "font-weight: bold"}},
						  {field: "LocationType",title: "Location Type",width: 120,headerAttributes: {style: "font-weight: bold"},hidden:true},
						 {field: "Loc_From",title: "From Loc",width: 150,headerAttributes: {style: "font-weight: bold"},editable: "false"},
						 {field: "Loc_Dest",title: "Dest Loc",width: 110,headerAttributes: {style: "font-weight: bold"},editable: "false"},
						 { title:"<input name='grpsage' id='grpsage' class='grpsage' type='checkbox' data-bind='checked: grpsage' #= grpsage ?/> Sage Status",width:105,headerAttributes: {style: "font-weight: bold"} ,editable: "false",
				...
				var ssource = $('#SourceInput').val();
                    ...
                    $("<div/>").appendTo(e.detailCell).kendoGrid({
						...
						columns: [
							...
							footerTemplate: "<div style='text-align:right'> #: kendo.toString(sum, 'n2') # </div>"},
							{ field: "Qty",title:"Qty",width: "85px",attributes: { style: "text-align: Right; font-size: 14px"},
							footerTemplate: "<div style='text-align:right'> #: kendo.toString(sum, 'n2') # </div>"},
				...
				function inserttoarray() {
								...
								for (var i = 0; i < ds.length; i++) {
									var dataItem = $("#grid").data("kendoGrid").dataSource.data()[i];
										...
										if(dataItem.chk == 'checked')
										{
													...
													var qLocImport = ds[i].qLocImport;
						...
						function clearlock()
						{
							...
							$.ajax({
									 url: "<?php echo base_url(); ?>cmovementAcc/clearlock",
									 type: 'get',
									 data:{},
									 dataType: "json",
									 success: function(data)
							...
							function updateqrnumber() {
								...
								for (i = 0; i < ds.length; i++) {
									...
									if (checkbox.is(":checked")) 
									{
											...
											if (qProdCode != '' && qPO != '' && Loc_Dest != '' && SageMoveId != '')
												{
													if (flag==0 && i > 1)
													{
														$("#myProgress").show();
													...
													$.ajax({
															url: "<?php echo base_url(); ?>cmovementAcc/loadqrnumber",
															type: 'get',
															data:{qProdCode:qProdCode,qPO:qPO,Loc_Dest:Loc_Dest,tgl:tgl,SageMoveId:SageMoveId,qhour:qhour,Loc_From:Loc_From},
															dataType: "json",
															success: function(data)
																{
																	//DoEvent(width,ds);
 ...
 <div id="content" style="margin-top:-20px">
    ...
    <div class="box-content">
											...
											<div class="box-tools" style="float:right;margin-right:0px;margin-top:5px">
											...
											<div class="box-tools" style="float:right;margin-right:5px;margin-top:0px">
														<div class="input-group" style="width: 85px; margin-top:0px; padding-right:-10px">
															...
															<select name="SourceInput" id="SourceInput" class="form-control input-sm pull-right" style="margin-top:0px;height:30px">
											...
											<div class="box-tools" style="float:right;margin-right:-8px;margin-top:5px">
														
														<div class="input-group" style="width: 60px; margin-top:0px; margin-left:3px">
															<span class="input-group" style="width: 60px; margin-top:0px; margin-right:3px"> Source
					...
					<div class="box-tools">
                     <form id="form2" name="form2" method="post" action="<?php echo base_url();?>cmovementAcc/tampil"  >
      
                    <div class="" style="width: 80px; margin-left:0px; ">
